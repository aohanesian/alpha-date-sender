...
router.post('/login', async (req, res, next) => {
    try {
        ...
        const whitelistResponse = await axios_1.default.get('https://firestore.googleapis.com/v1/projects/alpha-a4fdc/databases/(default)/documents/operator_whitelist');
        const whitelistedEmails = whitelistResponse.data.documents?.[0]?.fields?.email?.arrayValue?.values?.map(item => item.stringValue.toLowerCase()) || [];
