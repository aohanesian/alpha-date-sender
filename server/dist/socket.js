...
async function updateProfileOnlineStatus(socket, profileId) {
    try {
        ...
        const response = await axios_1.default.post(API_CONFIG.endpoints.setProfileOnline, payload, {
            headers: {
        ...
        const checkClickResponse = await axios_1.default.post(API_CONFIG.endpoints.checkClick, checkClickPayload, {
            headers: {
                'Authorization': `Bearer ${user.alphaDateToken}`,
                'Content-Type': 'application/json'
            }
    ...
    io.on('connection', async (socket) => {
        ...
        if (user?.alphaDateToken && user?.profiles) {
            try {
                ...
                const response = await axios_1.default.post(API_CONFIG.endpoints.senderList, payload, {
                    headers: {
                        'Authorization': `Bearer ${user.alphaDateToken}`,
                        'Content-Type': 'application/json'
                    }
...
function findRecipientId(messages, profileId) {
...
async function fetchAllPages(endpoint, payload, token, operatorId, profileId, type) {
    const allItems = [];
    let currentPage = 1;
    ...
    while (hasMorePages) {
        ...
        while (!pageSuccess) {
            try {
                ...
                const response = await axios_1.default.post(endpoint, { ...payload, page: currentPage }, {
                    headers: {
...
async function filterAvailableItems(items, operatorId, profileId, type, blockedRecipients, user) {
    // First filter Alpha Date blocks
    const alphaDateFiltered = items.filter(item => item.female_block === 0 && item.male_block === 0);
    ...
    for (const item of alphaDateFiltered) {
        try {
            ...
            const chatHistoryResponse = await axios_1.default.post(API_CONFIG.endpoints.chatHistory, chatHistoryPayload, {
                headers: {
                    'Authorization': `Bearer ${user.alphaDateToken}`,
                    'Content-Type': 'application/json'
                }
...
async function sendMessage(endpoint, payload, token, type, operatorId, profileId, messageHash, recipientId) {
    let retryCount = 0;
    while (true) {
        try {
...
async function processChatMessages(socket, profileId, message, operatorId) {
    ...
    try {
        ...
        while (true) {
            ...
            try {
                // 1. Fetch all pages
                const allChats = await fetchAllPages(API_CONFIG.endpoints.chatListByUserID, {
                    user_id: profileId,
                    chat_uid: false,
                    freeze: true,
                    limits: 1,
                    ...
                    SEARCH: "",
                    CHAT_TYPE: "CHANCE"
                }, user.alphaDateToken, operatorId, profileId, 'chat');
                ...
                const blockedRecipients = await redisService_1.redisService.getBlockedRecipientsSet(operatorId, profileId, 'chat');
                console.log(`🚫 Loaded blocklist with ${blockedRecipients.size} blocked recipients`);
                // 3. Filter available chats
                const availableChats = await filterAvailableItems(allChats, operatorId, profileId, 'chat', blockedRecipients, user);
                ...
                for (const chat of availableChats) {
                    ...
                    while (!messageProcessed && retryCount < 3) {
                        try {
                            ...
                            const response = await axios_1.default.post(API_CONFIG.endpoints.sendMessage, messagePayload, {
                                headers: {
                                    'Authorization': `Bearer ${user.alphaDateToken}`,
                                    'Content-Type': 'application/json'
                                }
                            ...
                            if (response.status === 200 && response.data?.error !== API_CONFIG.errors.RESTRICTION_ERROR_MESSAGE) {
                                ...
                                await redisService_1.redisService.markMessageSent(operatorId, profileId, chat.recipientId, 'chat', messageHash);
                                sentCount++;
                                totalSent++;
...
async function processMailMessages(socket, profileId, message, operatorId) {
    ...
    try {
        ...
        while (true) {
            ...
            try {
                // 1. Fetch all pages
                const allMailItems = await fetchAllPages(API_CONFIG.endpoints.chatListByUserID, {
                    user_id: profileId,
                    chat_uid: false,
                    freeze: true,
                    limits: 2,
                    ...
                    SEARCH: "",
                    CHAT_TYPE: "CHANCE"
                }, user.alphaDateToken, operatorId, profileId, 'mail');
                // 2. Load blocklist once for this cycle
                ...
                const blockedRecipients = await redisService_1.redisService.getBlockedRecipientsSet(operatorId, profileId, 'mail');
                console.log(`🚫 Loaded blocklist with ${blockedRecipients.size} blocked recipients`);
                // 3. Filter available mail items
                const availableMailItems = await filterAvailableItems(allMailItems, operatorId, profileId, 'mail', blockedRecipients, user);
                ...
                for (const mail of availableMailItems) {
                    ...
                    while (!messageProcessed && retryCount < 3) {
                        try {
                            ...
                            const mailSendResponse = await axios_1.default.post(API_CONFIG.endpoints.sendMail, mailSendPayload, {
                                headers: {
                                    'Authorization': `Bearer ${user.alphaDateToken}`,
                                    'Content-Type': 'application/json'
                                }
                            ...
                            if (mailSendResponse.status === 200 && mailSendResponse.data?.error !== API_CONFIG.errors.RESTRICTION_ERROR_MAIL) {
                                // Add recipient to blocklist after successful send
                                await redisService_1.redisService.markMessageSent(operatorId, profileId, mail.recipientId, 'mail', messageHash);
                                sentCount++;
                                totalSent++;
...
async function getAttachments(profileId, token) {
    try {
        const types = ['images', 'videos', 'audios'];
        const allAttachments = [];
        for (const type of types) {
            const response = await axios_1.default.get(`${process.env.ALPHA_DATE_API_URL}/files/${type}?external_id=${profileId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
...
async function updateProcessingProgress(operatorId, profileId, type, progress) {
